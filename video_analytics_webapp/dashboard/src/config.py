...
local_video_folder = gcs_bucket_url +'/local/data'
local_tmp_folder = gcs_bucket_url +'/local/tmp'
...
paths_to_make = [
    FileUtil.join(local_tmp_folder, video_frames_json),
    FileUtil.join(local_tmp_folder, video_frames_folder),
    FileUtil.join(local_tmp_folder, image_crops_frames),
    FileUtil.join(local_tmp_folder, audio_folder),
]
...
def clean_folders():
    for path in paths_to_make:
        for file in os.listdir(path):
            file_path = FileUtil.join(path, file)
